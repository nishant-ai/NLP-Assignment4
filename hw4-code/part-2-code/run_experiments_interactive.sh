...
# Interactive experiment runner for T5 fine-tuning
# Run this inside your interactive GPU session
...
# Start logging
echo "==================================================" | tee $LOG_FILE
echo "T5 Fine-tuning Experiments - Started at $(date)" | tee -a $LOG_FILE
echo "==================================================" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
run_experiment() {
    ...
    if [ ! -z "$last_result" ]; then
        ...
        echo "" | tee -a $LOG_FILE
        echo "[DONE] Completed: $exp_name (${RUNTIME}m)" | tee -a $LOG_FILE
