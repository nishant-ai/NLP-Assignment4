...
def create_augmented_dataloader(args, dataset):
    ...
    # Step 2: Randomly sample 5,000 examples for augmentation
    augment_subset = original_train.shuffle(seed=42).select(range(5000))

    # Step 3: Apply transformation to create augmented examples
    augmented_data = augment_subset.map(custom_transform, load_from_cache_file=False)

    # Step 4: Concatenate original training data with augmented data
    # This gives us 25,000 original + 5,000 augmented = 30,000 total examples
    combined_dataset = concatenate_datasets([original_train, augmented_data])
    ...
    # Step 6: Create DataLoader with the augmented dataset
